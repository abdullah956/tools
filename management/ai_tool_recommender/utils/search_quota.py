"""Utility functions for AI Tool Recommender search quota management."""
...
def get_user_search_quota(user) -> Dict[str, int]:
    """Get the user's remaining search quota for trial and subscription.
...
def decrement_user_search_count(user) -> Tuple[bool, str]:
    """Decrement the user's search count based on their subscription status.
    ...
    if active_subscription:
        ...
        logger.info(f"User {user.email} has active subscription - allowing search")
    ...
    if user.trial_searches > 0:
        ...
        logger.info(
            f"Decremented trial searches for {user.email}. "
            f"Remaining: {user.trial_searches}"
        )
        return True, f"Trial search used. Remaining: {user.trial_searches}"

    # No searches remaining
    logger.warning(f"User {user.email} has no remaining searches")
    return False, "No remaining searches. Please subscribe to continue."


def check_search_permission(user) -> Tuple[bool, str, Dict[str, int]]:
    """Check if user has permission to perform a search.
    ...
    """
    remaining = get_user_search_quota(user)
