...
if REDIS_URL:
    ...
    REDIS_CONFIG = {
        ...
        "socket_keepalive_options": {},  # OS-level keepalive
...
class RedisCache:
    ...
    async def get(self, key: str) -> Optional[Any]:
        """Get data from cache with retry logic."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Cache get error (attempt {attempt + 1}/{max_retries}): {e}, retrying..."
                    )
                    ...
                    logger.error(f"Cache get error after {max_retries} attempts: {e}")
                    return None
            except Exception as e:
    ...
    async def set(self, key: str, data: Any, ttl: int = 300) -> bool:
        """Set data in cache with retry logic."""
        if not self.redis_client:
            return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Cache set error (attempt {attempt + 1}/{max_retries}): {e}, retrying..."
                    )
                    await asyncio.sleep(0.5 * (attempt + 1))  # Exponential backoff
                    # Try to reconnect
                    try:
                    ...
                    logger.error(f"Cache set error after {max_retries} attempts: {e}")
                    return False
            except Exception as e:
