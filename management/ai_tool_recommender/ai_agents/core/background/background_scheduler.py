"""Django Background Scheduler for AI Tool Recommender."""
...
class BackgroundScheduler:
    """Django Background Scheduler for handling async tasks."""
    ...
    def add_task(
        ...
        **kwargs,
        ...
        if delay > 0:
            self.executor.submit(self._delayed_task_wrapper, task_id, delay)
        else:
            self.executor.submit(self._task_wrapper, task_id)

        logger.info(f"Added background task: {task_id}")
        return task_id

    def _delayed_task_wrapper(self, task_id: str, delay: float):
    ...
    def _execute_task_sync(self, task_id: str):
        """Execute a background task synchronously."""
    ...
    def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        for key in ["created_at", "started_at", "completed_at"]:
            if key in task_info and isinstance(task_info[key], datetime):
...
async def background_generate_workflow(
    ...
    try:
        ...
        from ai_tool_recommender.ai_agents.tools.ai_tool_recommender import (
            AIToolRecommender,
        )
...
async def background_add_new_tools_to_pinecone(tools: list, query: str = ""):
    """Background task to trigger FULL SCRAPING PIPELINE for internet-discovered tools."""
    try:
        logger.info(
            f"🔄 BACKGROUND TASK: Processing {len(tools)} internet search tools for SCRAPING"
        )
        ...
        # Import here to avoid circular imports
        from ai_tool_recommender.ai_agents.core.discovery import tool_discovery_service
        ...
        for i, tool in enumerate(tools, 1):
            try:
                ...
                if not is_existing:
                    logger.info(
                        f"✅ NEW TOOL: '{title}' - not found in Pinecone, will scrape"
                    )
        ...
        logger.info(f"   🆕 New tools to scrape: {len(new_tools)}")
        logger.info(f"   🔄 Duplicates skipped: {skipped_count}")

        # Trigger FULL SCRAPING PIPELINE for new tools
        if new_tools:
            logger.info(
                f"🚀 Triggering FULL SCRAPING PIPELINE for {len(new_tools)} new tools..."
            )
            ...
            # Create scraping job
            job = await sync_to_async(ScrapingJob.objects.create)(
                ...
                payload={
                    ...
                    "source_query": query,
                    "tools": new_tools,
                    "url_count": len(new_tools),
                },
            )

            logger.info(f"✅ Created ScrapingJob {job.id} for {len(new_tools)} tools")

            # Trigger Celery task (same as CSV upload pipeline)
            process_internet_discovered_tools.delay(str(job.id))

            logger.info(f"🔄 Celery task queued for job {job.id}")
            logger.info(
                f"📝 Tools will be: scraped → LLM extracted → indexed to Pinecone"
            )

            added_count = len(new_tools)
            duplicate_count = skipped_count

            logger.info("✅ SCRAPING PIPELINE TRIGGERED:")
            logger.info(f"   ✅ Tools queued for scraping: {added_count}")
