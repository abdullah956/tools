        ...
        try:
            ...
            for query in self.discovery_queries[:5]:  # Limit to 5 queries per run
                try:
                    ...
                    search_results = await self.internet_service.search_tools(
                        query=query,
                        max_results=10,
                        validate_tools=True,
                    ...
                    if search_results and search_results.get("tools"):
            ...
            for search_query in search_queries[
                ...
                try:
                    search_results = await self.pinecone_service.search_tools(
                        ...
                        similarity_threshold=0.7,  # Lower threshold for better detection
                    )

                    if search_results and search_results.get("tools"):
                        existing_tools = search_results["tools"]
