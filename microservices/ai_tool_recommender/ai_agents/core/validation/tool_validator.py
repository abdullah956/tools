...
from microservices.ai_tool_recommender.ai_agents.core.prompt_system import (
    PromptProcessor,
    ToolDataEnhancer,
)
...
class ToolDataValidator:
    ...
    async def validate_tool_data(self, tool: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Enhanced tool data with all required fields filled
        """
        try:
            ...
            # Return tool with minimal required fields as fallback
            return self._create_fallback_tool(tool)
    ...
    async def validate_tools_batch(
        ...
        for i, result in enumerate(validated_tools):
            if isinstance(result, Exception):
                ...
                final_tools.append(self._create_fallback_tool(tools[i]))
            else:
    ...
    def _create_fallback_tool(self, tool: Dict[str, Any]) -> Dict[str, Any]:
        """Create a tool with minimal required fields as fallback."""
        fallback_tool = tool.copy()
        fallback_tool["Title"] = tool.get("Title", "Unknown Tool")
        fallback_tool["Description"] = tool.get("Description", "AI tool")
